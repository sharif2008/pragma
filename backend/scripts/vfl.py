...
def load_primary_domains(json_path: Path | None = None) -> dict[str, list[str]]:
    """Attack -> preferred network_tier labels from attack_options.json."""
    data = load_attack_options(json_path or _attack_options_path())
    raw = data.get("primary_domains") if isinstance(data, dict) else None
    if not isinstance(raw, dict):
        return {}
    out: dict[str, list[str]] = {}
    for key, values in raw.items():
        if isinstance(values, list):
            out[str(key).upper()] = [str(x) for x in values]
    return out


def load_domain_capabilities(json_path: Path | None = None) -> dict[str, list[str]]:
    """Canonical domain label -> action_capabilities from agentic_features.json."""
    from scripts.network_domains import DOMAIN_LABELS, STORAGE_BUCKETS, STORAGE_TO_DOMAIN

    caps = {d: [] for d in DOMAIN_LABELS}
    path = Path(json_path or AGENTIC_FEATURES_JSON)
    if not path.is_file():
        return caps
    defs = load_agent_definitions(path)
    for bucket, acts in zip(STORAGE_BUCKETS, defs.get("agent_actions") or []):
        domain = STORAGE_TO_DOMAIN.get(str(bucket))
        if domain:
            caps[domain] = [str(x) for x in acts]
    return caps


def load_pipeline_catalogs() -> tuple[dict[str, list[str]], dict[str, list[str]], dict[str, list[str]]]:
    """Whitelist W[a], domain capabilities, and primary domains."""
    attacks = {k: list(v) for k, v in load_attack_actions_by_type().items()}
    return attacks, load_domain_capabilities(), load_primary_domains()
...
generate_agent_name = generate_party_name
...
get_agent_actions_for_attack = get_party_actions_for_attack
